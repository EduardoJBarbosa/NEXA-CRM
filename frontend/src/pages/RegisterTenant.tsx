import { useState } from 'react'
import { useNavigate } from 'react-router-dom'
import { useForm } from 'react-hook-form'
import { zodResolver } from '@hookform/resolvers/zod'
import { z } from 'zod'
import { useAuthStore } from '@/stores/authStore'
import api from '@/services/api'
import { AlertCircle, Loader, ArrowLeft } from 'lucide-react'
import { APP_NAME } from '@/utils/constants'

const registerSchema = z.object({
  tenantName: z.string().min(1, 'Nome do tenant obrigatório'),
  tenantSlug: z
    .string()
    .min(1, 'Slug obrigatório')
    .regex(/^[a-z0-9-]+$/, 'Slug deve conter apenas letras, números e hífen'),
  adminName: z.string().min(1, 'Nome do admin obrigatório'),
  adminEmail: z.string().email('Email inválido'),
  adminPassword: z.string().min(6, 'Senha deve ter no mínimo 6 caracteres'),
  confirmPassword: z.string(),
}).refine((data) => data.adminPassword === data.confirmPassword, {
  message: 'Senhas não conferem',
  path: ['confirmPassword'],
})

type RegisterForm = z.infer<typeof registerSchema>

export default function RegisterTenant() {
  const navigate = useNavigate()
  const { setToken, setUser } = useAuthStore()
  const [error, setError] = useState('')
  const [loading, setLoading] = useState(false)

  const { register, handleSubmit, formState: { errors } } = useForm<RegisterForm>({
    resolver: zodResolver(registerSchema),
  })

  const onSubmit = async (data: RegisterForm) => {
    try {
      setError('')
      setLoading(true)

      const response = await api.post('/api/tenants/register', {
        tenantName: data.tenantName,
        tenantSlug: data.tenantSlug,
        adminName: data.adminName,
        adminEmail: data.adminEmail,
        adminPassword: data.adminPassword,
      })

      const { token, user } = response.data

      if (!token || !user) {
        setError('Resposta inválida do servidor')
        setLoading(false)
        return
      }

      setUser(user)
      setToken(token)

      setTimeout(() => {
        navigate('/dashboard', { replace: true })
      }, 100)
    } catch (err: any) {
      setError(err.response?.data?.error || 'Erro ao registrar tenant')
      setLoading(false)
    }
  }

  return (
    <div className="min-h-screen bg-gradient-to-br from-nexa-dark via-nexa-bg to-nexa-dark flex items-center justify-center p-4">
      <div className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-2xl shadow-2xl p-8 w-full max-w-md">
        <button
          onClick={() => navigate('/login')}
          className="flex items-center gap-2 text-gray-300 hover:text-white mb-6 transition-colors"
        >
          <ArrowLeft size={18} />
          Voltar ao Login
        </button>

        <div className="mb-8 flex flex-col items-center">
          <h1 className="text-4xl font-bold bg-gradient-to-r from-violet-400 to-cyan-400 bg-clip-text text-transparent mb-2 text-center">
            {APP_NAME}
          </h1>
          <p className="text-gray-300 text-center">Registre sua Clínica</p>
        </div>

        {error && (
          <div className="bg-red-500/20 border border-red-500/50 rounded-lg p-4 mb-6 flex items-gap gap-2">
            <AlertCircle size={20} className="text-red-400 flex-shrink-0" />
            <p className="text-red-200 text-sm">{error}</p>
          </div>
        )}

        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-200 mb-2">Nome da Clínica</label>
            <input
              {...register('tenantName')}
              type="text"
              placeholder="Minha Clínica Odontológica"
              className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent text-white placeholder-gray-400 transition-colors"
            />
            {errors.tenantName && (
              <p className="text-red-400 text-sm mt-1">{errors.tenantName.message}</p>
            )}
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-200 mb-2">Slug da Clínica</label>
            <input
              {...register('tenantSlug')}
              type="text"
              placeholder="minha-clinica"
              className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent text-white placeholder-gray-400 transition-colors lowercase"
            />
            {errors.tenantSlug && (
              <p className="text-red-400 text-sm mt-1">{errors.tenantSlug.message}</p>
            )}
          </div>

          <div className="border-t border-white/10 pt-4 mt-6">
            <h3 className="text-sm font-medium text-gray-200 mb-4">Dados do Administrador</h3>
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-200 mb-2">Nome do Admin</label>
            <input
              {...register('adminName')}
              type="text"
              placeholder="João Silva"
              className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent text-white placeholder-gray-400 transition-colors"
            />
            {errors.adminName && (
              <p className="text-red-400 text-sm mt-1">{errors.adminName.message}</p>
            )}
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-200 mb-2">Email</label>
            <input
              {...register('adminEmail')}
              type="email"
              placeholder="admin@clinica.com"
              className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent text-white placeholder-gray-400 transition-colors"
            />
            {errors.adminEmail && (
              <p className="text-red-400 text-sm mt-1">{errors.adminEmail.message}</p>
            )}
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-200 mb-2">Senha</label>
            <input
              {...register('adminPassword')}
              type="password"
              placeholder="••••••••"
              className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent text-white placeholder-gray-400 transition-colors"
            />
            {errors.adminPassword && (
              <p className="text-red-400 text-sm mt-1">{errors.adminPassword.message}</p>
            )}
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-200 mb-2">Confirmar Senha</label>
            <input
              {...register('confirmPassword')}
              type="password"
              placeholder="••••••••"
              className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent text-white placeholder-gray-400 transition-colors"
            />
            {errors.confirmPassword && (
              <p className="text-red-400 text-sm mt-1">{errors.confirmPassword.message}</p>
            )}
          </div>

          <button
            type="submit"
            disabled={loading}
            className="w-full bg-gradient-to-r from-violet-600 to-cyan-600 text-white font-medium py-3 rounded-lg hover:shadow-lg hover:shadow-violet-500/50 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {loading && <Loader size={18} className="animate-spin" />}
            {loading ? 'Registrando...' : 'Registrar Clínica'}
          </button>
        </form>
      </div>
    </div>
  )
}
